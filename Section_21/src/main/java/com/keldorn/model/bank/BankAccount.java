...
public class BankAccount {
    ...
    public void deposit(double amount) {
        ...
        synchronized (lockBalance) {
            ...
            balance += amount;
            System.out.printf("STATING BALANCE: %.0f, DEPOSIT (%.0f) : NEW BALANCE = %.0f%n",
    ...
    public synchronized void withdraw(double amount) {
        ...
        if (amount <= balance) {
            balance -= amount;
            System.out.printf("STATING BALANCE: %.0f, WITHDRAWAL (%.0f) : NEW BALANCE = %.0f%n",
                    origBalance, amount, balance);
        } else {
            System.out.printf("STATING BALANCE: %.0f, WITHDRAWAL (%.0f) : INSUFFICIENT FUNDS!%n",
                    origBalance, amount);
        }
    }
}
