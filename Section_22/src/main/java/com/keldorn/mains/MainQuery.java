...
//        String jpql = "SELECT a FROM Artist a WHERE a.artistName LIKE ?1"; // :partialName could be used instead of ?1
        String jpql = "SELECT a FROM Artist a JOIN albums album " +
                      "WHERE album.albumName LIKE ?1 OR album.albumName LIKE ?2";
        var query = em.createQuery(jpql, Artist.class);
        query.setParameter(1, matchedValue); // query.setParameter("partialName", matchedValue);
